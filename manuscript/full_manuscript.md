...
| --- | --- |
| ファイル形式 | すべてPNG形式 |
| 縦横のピクセル数 | 自動的に縮小されるため**偶数**にする |
| 解像度 | **72dpi以上** |
| カラーモード | **RGB** |
| 1個あたりのファイルサイズ | **1MB以下** |
| ZIPでまとめる場合 | ZIPファイルを**60MB以下** |
| 背景 | 透過にする |
| 余白 | トリミングされた画像の外枠とコンテンツの間に**10pxぐらいの余白が必要** |
...
```text
【スクリーンショット挿入】※ 取得済み → assets/screenshots/public/ch01_03_guideline-sticker.png
画面：LINE Creators Market スタンプ制作ガイドライン
撮影内容：画像サイズの表と、余白・解像度・カラーモードの条件が書かれた部分
注意：URLが見えるようにキャプションを添える（読者が自分で確認できるように）
```
...
| サイズの追加条件 | 横・縦のどちらかが**270px以上**であること |
| フレーム数 | **1個あたり5〜20フレーム** |
| 余白 | **余白はつけない**（静止画とは逆） |
...
また、LINE STOREやスタンプショップでは**1フレーム目が静止画として表示される**ため、1枚目の絵が最も重要になります。

> 仕様は変更される可能性があります。**申請前にLINE Creators Market公式の最新ガイドラインを確認してください。**

```text
【スクリーンショット挿入】※ 取得済み → assets/screenshots/public/ch10_01_guideline-animationsticker.png
画面：アニメーションスタンプ制作ガイドライン
撮影内容：サイズ・個数・フレーム数の条件が書かれた部分
注意：URLが見えるようにキャプションを添える
```
...
- 1個あたり1MB以下 / ZIPは**20MB以下**（スタンプの60MBとは違う）
...
```text
【スクリーンショット挿入】※ 取得済み → assets/screenshots/public/ch10_02_guideline-emoji.png
画面：絵文字 制作ガイドライン
撮影内容：パッケージタイプごとの必要数とサイズの表
注意：URLが見えるようにキャプションを添える
```
